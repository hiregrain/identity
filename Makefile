...
.PHONY: check check-red check-red-db metadata install lint fmt-check go-check ts-check db-up db-reset migrate migrate-verify typegen typegen-check append-only spine-schema payload-residency scored-columns ledger-stamp-grant two-plane-split envelope-test cross-plane-constructs cross-plane-outbox person-test keylog-test signing-test streams-test deletion-test reference-test differential differential-red db-down vectors vectors-check kernel-budget kernel-governance

check: metadata install lint go-check signing-test reference-test differential db-up migrate-verify typegen-check append-only spine-schema payload-residency scored-columns ledger-stamp-grant two-plane-split envelope-test cross-plane-constructs cross-plane-outbox person-test keylog-test streams-test deletion-test ts-check
	$(COMPOSE) down
	@echo "check: green"
...
# corpus plus fresh randomized input, compared byte for byte. This is the
# criterion no golden vector can discharge, because the vectors are
# generated by the implementation they test and agree with whatever bug
# the generator had; two implementations written from the contract by
# parties that did not read each other do not.
#
# In `check` since both sides exist: the adapter landed with
# trust-kernel/01 and the whole run costs about nine seconds, so there is
# no argument for keeping the layer's hardest criterion out of the
# standing gate. The seed is fresh every run and printed, so a divergence
# is replayable from the failure output; `differential-red` is the fixed
# seed, because a red path that drifts is not a proof.
#
# KERNEL_ADAPTER is any command speaking the protocol documented at the
# top of reference/adapter.ts. Overridable so a built binary can stand in
# for `go run` where compile time matters.
KERNEL_ADAPTER ?= cd core && go run ./cmd/kernel-adapter
...
differential-red:
	...
	node reference/harness/differential.ts \
		...
		--impl kernel="$(KERNEL_ADAPTER)" \
		--fresh 500 --seed 7
	@echo "differential-red: every planted misreading was caught on either side"
