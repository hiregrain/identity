...
func TestAppendRoundTrip(t *testing.T) {
	...
	events, err := keylog.EventsForKey(ctx, key)
	if err != nil {
		t.Fatalf("EventsForKey: %v", err)
	}
	if len(events) != 1 {
...
func TestTheCompromiseCutHoldsOverRowsReadBack(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			valid, err := keylog.SignatureValid(ctx, key, c.stamp)
			if err != nil {
...
func TestRotationClosesTheWindowOverRowsReadBack(t *testing.T) {
	...
	valid, err := keylog.SignatureValid(ctx, key, rotated.Add(-time.Minute))
	if err != nil {
		t.Fatalf("SignatureValid: %v", err)
	}
	if !valid {
	...
	valid, err = keylog.SignatureValid(ctx, key, rotated)
	if err != nil {
		t.Fatalf("SignatureValid: %v", err)
	}
	if valid {
...
func TestAKeyWithNoEventsIsNeverValid(t *testing.T) {
	ctx := context.Background()
	valid, err := keylog.SignatureValid(ctx, newKeyID(t), time.Now().UTC())
	if err != nil {
		t.Fatalf("SignatureValid: %v", err)
	}
	if valid {
