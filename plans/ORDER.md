...
| **Whether the consent instrument can be split without weakening the consent Grain relies on** (decision 058). Just-in-time is better comprehension and, on its face, stronger consent; that is a legal question and `counsel/` holds questions put, not advice received | `consent-and-deletion` going `ready` | open, counsel, brief to write |
