...
3. **The store privacy declarations match the consent instrument line by
   ...
   036 and 038 by a diff rather than by reading. A declaration that drifts
   from the consent instrument is the failure mode this criterion exists to
   catch.
4. **Deletion satisfies Apple and Play, which require different things.** An
   in-app control that *initiates* deletion (Apple 5.1.1(v); support-only is a
   rejection) and a web URL where deletion can be requested (Play; no in-app
   change satisfies it).
