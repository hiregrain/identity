...
worker-permissioned design does **not** weaken CRA status, because each of those
...
- FCRA §1681i(a)(5)(A): after reinvestigation, inaccurate/unverifiable
  information **must be deleted or modified**, so a superseding annotation is
  not enough.
- FCRA §1681c(a)(5): adverse items generally cannot be reported after seven
  years, so a permanent, fully readable record violates this on its face.
- PH DPA IRR Sec. 34(e): erasure trigger for "private information prejudicial
  to the data subject", which describes an adverse attestation almost exactly,
...
both stay accessible and be delivered together to recipients, meaning versioned
...
5. **A reputation network survives only with a compelled consumer.** `02`:
   ...
   sequencing, since the verticals are the compelled consumers. Standalone-identity
...
7. **Issue an opaque `ledger_person_id` at self-signup; verify progressively;
   ...
   the Princeton study), so grain's phone-anchor ADR should not generalize.
...
8. **Author a small proprietary `work_kind` vocabulary; crosswalk out.**
   ...
   transferable prior art, and it is a real open design problem.
