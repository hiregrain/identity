# Pre-Claim Employer-Provided Records — United States
...
transmit active roster data — identifiers plus employment facts — about workers who are not
...
imposes duties — status registration, notice, rights, retention limits — that begin when
...
sitting entity that does exactly this mechanic — employer pushes roster/payroll records about
all employees, platform furnishes to third parties — operates as a registered CRA: Equifax's
The Work Number, Truework, Pinwheel. Every entity that credibly claims non-CRA status in the
adjacent market — Argyle is the clean example — rests that claim on the consumer initiating
...
own terms on 1 Jan 2023 — they were not repealed, and § 1798.145(m)(4) and (n)(3) still say so —
so employee data is fully in scope. "Consumer" is any California resident (§ 1798.140(i)); no
...
excuses a **registered data broker** collecting indirectly, and — the route the corpus would not
have found — **Civ. Code § 1798.100(b)**, which lets a business that "acting as a third party,
...
**Personal phone and email fall outside that carve-out** — they are individually identifiable
...
are exact: **§ 7060(b) forbids verifying an opt-out request at all** — the one request Grain must
...
employer, which — with *Raines*, 15 Cal.5th 268 (2023), and *Mobley v. Workday* (ADEA collective
certified May 2025, applicant coverage upheld March 2026) — gives three independent routes to
...
- **§ 7012(g)** — where more than one business controls collection, **both** must give notice,
  ...
  route, and it is worse than it sounds: it requires every connected employer's HR notice to
...
Confidence: high — all three subsections verified against the approved regulatory text.
...
own right with respect to the ledger it builds from it. Confidence: high** — the disqualification
is textual, not judgmental.
...
attempting service-provider language the facts contradict — a contract contradicted by the facts
is worse than no contract in an enforcement posture.
...
American law treats as least protected — it is the residue that survived the defamation-driven
...
exception 1. **Personal phone and email do not** — they are individually identifiable
...
Exception 2 is Grain's best argument at ingest — a vendor performing employment-related services
for the employer — but it is drafted around a service relationship running *to the supplying
...
services — the search located none either way, so treat this as genuinely untested rather than
...
Compelled self-publication — publication satisfied where the plaintiff was compelled to repeat
the statement and the compulsion was foreseeable to the defendant — is recognised in a minority
...
enforceable** — it is the workhorse of furnisher litigation. So if Grain is a CRA, **every
...
Reuters escaped on an elaborate anti-FCRA-use apparatus — subscriber vetting, contractual
prohibitions, purpose certification on *every single search*, training, auditing, misuse
...
Note the case name: it is *United States v. Spokeo*, DOJ suing on FTC referral for civil
...
A retroactive-ratification argument — the claim event cures the pre-claim window — fails for
...
Separately, the word "solely" does independent work: a single non-experience field — a score, a
cross-employer standing derivation — contaminates the whole communication even before the privity
point is reached.
...
model.** (1) complete a transaction or perform a contract *with the consumer* — unavailable, a
contract with the employer is not one with the consumer; (2) security and integrity — narrow;
(3) debugging — no; (4) free speech or another legal right — the only non-trivial argument and
still weak, because the data is privately transmitted rather than public; (5) CalECPA — no;
(6) research — unavailable, it requires the consumer's informed consent; (7) **solely internal
uses aligned with the consumer's expectations based on the consumer's relationship with the
business** — unavailable, and instructively so: "solely internal" excludes a sold product, the
relationship is null, and the consumer never provided the data; (8) legal obligation — only for
genuine retention mandates, and § 5.3 finds none.
...
The obvious objection — how does a person exercise rights over a record they do not know
...
that is the defining feature of the mechanic — and it discloses the record to employers for
...
- **Vermont — 9 V.S.A. § 2446.** § 2430(4) uses the direct-relationship formulation, and its
  ...
  is the trap: the worker is an employee of the employer, not of Grain**, so no direct
...
not audit its own product even voluntarily. Vendor exposure is contractual — the audit needs
Grain's cooperation and data — not statutory. One development worth tracking: in **December 2025
...
- **Four-year recordkeeping**, expressly including ADS data — data provided by or about
...
- **Anti-bias testing is relevant evidence** — its quality, scope, recency, and the response to
  results — and its *absence* is probative. The asymmetry matters: testing and then not acting on
...
wrong with the model. Confidence: medium — the mechanism is well-grounded, the empirical
...
alleged recipients could re-identify contributors anyway** — that allegation is the heart of the
...
wage-fixing trial conviction on 14 April 2025** — a home-healthcare staffing executive who
...
the sensitive category in every authority above — the plus factors that push toward per se
...
is defensible as an efficiency — it reduces fraud in employment claims. A cross-employer score
...
dangerous single object in the schema**: it is a cross-employer *normalised seniority signal*,
...
the mechanic as described — *Todd*'s structural factors are all present. Medium on ultimate
...
3. **Cap the pre-claim payload at dates, title, role and employment status.** No outcome facts,
   ...
   **Then handle the identifiers separately, because they are the exposed element.** Phone and
   ...
   directly with constraint 10 — that tension should be resolved deliberately rather than by
...
10. **Notify the subject, even though no statute clearly requires it here.** A message to the
    employer-supplied contact at first write — "your employer recorded your role and dates with
    Grain; here is how to claim, see, or delete it" — collapses most of this memo's exposure at
