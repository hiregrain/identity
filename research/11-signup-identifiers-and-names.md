...
| **Aadhaar-linked Indian platforms** | Aadhaar number + OTP to the **Aadhaar-linked** mobile number (or biometric) | Yes, for eKYC | Note that the phone is not the identity, but a delivery channel for an OTP that authenticates against UIDAI. Biometric eKYC is the fallback for users whose mobile isn't linked. |
