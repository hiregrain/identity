...
at reference level — identifier and proposition, not decision text; EDPB
...
**1. There is no lawful basis for the mechanic as specified. Not "a weak basis" — none.**
...
*demonstrate* each individual consented — a contractual warranty from the employer is not
...
attestation creation — and does not extend to Grain's later cross-employer record,
...
(it depends on facts not yet fixed — how much the employer specifies about what is
recorded).
...
grounds which override the interests, rights and freedoms of the data subject" — a
...
the shape is well-supported, the number is a judgment call.
...
accountability** — the company "transferred the burden of proof of compliance to the data
...
alternative — it is, in fact, what decision 028 already ruled for the general product
...
- **No employer may read an unclaimed record. Ever. Including the employer that supplied
  it** — that employer already holds the underlying facts in its own HR system and gains
...
the 9(2)(b) conclusion — it is textual inference; no authority squarely on point was
found.**
...
(EDPB ¶68–72) rather than joint. That would be *worse* for Grain, not better — it would
...
Art. 14(2)(f) — source — is the one an aggregator habitually gets wrong. WP260 ¶60: the
...
Each controller owes its own Art. 13/14 duty. A generic HR clause — "we may share your
...
**Yes, an actively delivered claim invitation can discharge Art. 14 — it is the only
...
4. **Every right is exercisable from inside the message**, by a one-click link that does
   ...
   highest-value control in the design: it converts Art. 21(1) from a legal liability into
...
5. **In the language of the worker's country.** *KASPR* ¶88: CNIL rejected the argument
   ...
   under Art. 12. In hospitality this is not a marginal point — the workforce is
...
no — that is why *KASPR* is the closest thing and why this is the weakest-evidenced
...
from inactivity or restart on re-sync. **Medium** on 365 days specifically — it is a
...
Art. 8A(2)(b) — "the context in which the personal data was collected, including the
...
bites at the level of ordinary reasonableness: it is hard to call an SMS to a number you
...
- **Art. 22B(1)**: the restriction now applies only to significant decisions "based
  entirely or partly on processing described in **Article 9(1)**" — special category data
...
**Confidence: medium — the guidance is draft and the vetting/verification line may soften.
