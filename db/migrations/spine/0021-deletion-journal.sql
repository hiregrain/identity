...
-- DEK is destroyed — but a backup taken before the deletion still holds
-- the wrapped key, so a restore resurrects the person at the byte level.
...
-- channel, no content, no commitment, no reason text — nothing
...
-- them is retried forward — same recovery grammar as the cross-plane
...
--     a fact about this product's only two deletion flows, the same
--     for every row; risk accepted, it describes process, never the
--     person.
--
