...
| RF-2 | Worker initiation required — ingestion validation rejects any attestation with employer-roster provenance lacking a live `verification_request_id`; no write path accepts a roster row as a source of truth | any roster-derived write without the id | `ingestion` (AC-I5) |
...
| RF-5 | Invitation payload schema-constrained to employer identity plus addressing | any worker-specific field | `integration-surface` (AC-IS5) |
