...
1. **Identity and capability never share a record.** Capability belongs to a
   ...
   duplicates the identity, which is fatal to a system whose premise is that
   people move up.
...
3. **The evidence record is append-only and survives everything.** Promotion
   out, offboarding, and restriction are all transitions on a participation
...
10. **Portability is scoped, and the scope is a property of the record.**
    ...
    direct partners, not published externally. It is valuable enough to
    matter to a worker, scoped enough that the network still holds.
...
4. **Dimension standing: derived or attested?** Is standing on the
   ...
   fact each attestation asserts? (Dispatch's internal instinct, to derive
   and never store, is position 2 above, but the ledger operates at coarser
