...
**Main** is the core screen. Identity and imprint composed as one hero, then
outstanding verification, work history, sharing (decision 035, amending 028).
**Expanded** is the imprint at full size. Angle reads the measure, radius reads
the chapter. This is what closes `imprint/README.md` §7.1, the missing angular
anchor. **Landing** is an attestation arriving, the heavier of the two ceremonies
`DESIGN.md` §10 allows. **Consent** covers the seven mechanics. **Empty**,
**Imported**, and **Handle** are the first-run sequence. **Settings** is read-only,
per decision 036. **Public** is the page a worker sends someone, on its own
