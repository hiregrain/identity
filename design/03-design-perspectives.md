...
keeps only structure — pixel mosaic, dotted vectors, twice-printed
serial — and still reads unmistakably as money
...
- **Attestations carry role, not just name** — "verified by [party],
  [role], [date]" — and every verification mark resolves to evidence
...
X's restraint decomposes mechanically: no cards, no borders — 
hairlines and whitespace only — so the one boxed element (quote tweet)
...
worker whose career is on the page — order, stamp, grain, precision,
state? (2) cropped, could it be mistaken for either slop cluster?
