...
but to name the *mechanism* of convergence: AI-assisted design collapses
toward two attractor clusters, and when criticized for one it swings to
...
- Inter/generic grotesk at default weights with no typographic point of
  view.
- Confetti or sparkle emoji standing in for a designed celebration.
- Gradient text headlines; "AI shimmer" effects.
- Feature-grid marketing layout (three columns, icon, four words).
...
- Em-dash-cadence copywriting and marketing-speak inside the product.
- Any element whose only justification is "this looks designed."
- Uniform treatment: any single device (radius, border, weight, color)
  applied everywhere is a tell. Designed systems vary treatment by
  meaning.
...
- Motion that decorates instead of informs; celebration spent on
  unimportant moments (see the celebration economy in 00).

## Additions from visual round 1 (2026-08-17, founder-rejected renders)

Learned by rendering five full directions and having all five rejected:

- **Document cosplay**: rendering app surfaces as literal documents
  (certificate chassis, ruled registry pages, MRZ strips, passport
...
- **Identity motifs that need a legend** (unlabeled tally fields). If
  the signature element requires explanation, it has failed.
- **Generic-page syndrome**: a screen with no app anatomy (no nav, no
  affordances, no interactive structure) reads as a poster, not a
  product.
- **Thick left bars on callouts, gap boxes, quotes or notes.** Already
  listed in Cluster A; restated as an explicit permanent ban after it
  reappeared in a design-system proposal on 2026-08-18. A left rail is
  the laziest hierarchy device available and a reliable tell. Emphasis
  comes from weight, space and rule-work, never from a coloured bar
  running down the side of a block.
