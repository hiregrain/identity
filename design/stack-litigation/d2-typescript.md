...
1. **The real question is not "TS vs Go" — it is "one language vs two."** The
...
4. **TS's real weaknesses are conceded and priced, not denied.** Runtime type
   ...
   at every boundary, invariants revoked/CHECKed in Postgres itself — the
   convergence map's settled item 5 — and the production invariant auditor).
...
log-centric §8 — three independent convergences). The convergence map's
...
(ai-native §2). That is real setup cost — days, once — and agents follow
...
- **Merge/alias** (design §1.4): a subject reference is
  ...
  attestations keep their original `subject_id` forever and resolve through
  the alias closure at read time — means every read-path consumer must
...
- **The specs themselves are language-independent.** TLA+ models the
  merge/unmerge-vs-ingest and deletion-vs-read *protocols* — states,
  actions, invariants — not implementation code. Both specs are written,
...
5. **A stage-C hot-path ceiling is plausible:** packet assembly at 10⁹
   ...
   by-then-executable kernel spec) is a real, priced cost — ai-native §9
   says the same.
