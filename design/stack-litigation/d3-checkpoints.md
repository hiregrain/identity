...
compromised admin, a poisoned deploy, or an engineer under coercion
...
- Ingestion returns a signed receipt to the submitting party: entry
  ...
  within T. This is the SCT analog. Parties (and anyone the operator chooses to
...
Adversary: the operator itself, holding admin credentials, deploy
pipeline access, and database superuser rights. Goal: alter or excise attested history silently.
...
2. Omission is detectable by the injured party: they hold a signed
   ...
   Detection requires the party to check. This is a real limitation,
   conceded in §5.
...
start, as §4.2 shows, and the sentence security-infra prices ("our log
...
infra, and egress, all of it true, all of it real. The public instance runs at a
99.5% SLO.
...
- **Entry-model mismatch.** rekor-tiles entry types are software-supply-
  ...
  doesn't fit, run "Tessera-on-GCS/S3 with our own entry schema",
  meaning a bespoke log personality, written and maintained by a 3-person team,
...
- **Failure-domain placement.** That last clause is the expensive one.
  ...
  incident, an antispam/dedup edge case, or a botched shard rotation.
  Each is a production write outage *and* a public incident in the integrity
...
by a verifier who consumes inclusion/consistency proofs, such as a
sophisticated external auditor, a partner's security team, or a
regulator's technical consultant. Workers cannot read either; internal verticals
...
- **Trigger:** the earlier of (a) the first external attesting party's
  ...
  itself enforcing the ordering, since activation is a steward-gated
...
### 4.3 The genesis-consistency proof that makes the migration clean

At the trigger, the log does not start empty and it does not start
blind:

1. Replay the full historical spine (append-only by construction, since the
...
2. **Witnessing becomes purchasable for private logs before launch**,
   meaning a witness-network-as-a-service or ≥2 independent reputational units
...
5. **D1/D6 resolve toward infrastructure that already embeds a tiled
   log on the write path**, a marginal cost collapse per concession 5.
