...
that one author writing both sides shares the misreading twice, applies to this
...
Grain's product test, that every claim resolves to the evidence beneath it, is
not met if one commercial event can end both. OCI is preferred over Backblaze for
...
- **Hetzner, OVH bare metal, Fly.io, Railway, Render** fail D1 and the
  real-KMS rule.
- **Neon, Supabase, Timescale** have no commit-gating durability control; the
  deletion saga crosses an uncontrolled vendor boundary.
- **Crunchy Bridge** has real Postgres-specialist quality, but is rejected on
  the Snowflake acquisition. A system of record should not sit inside a
  recently-acquired product on a new owner's roadmap.
- **EDB (BigAnimal / Postgres AI Cloud)** is the strongest specialist; it runs
  in the customer's own AWS or Azure account so it adds no custody boundary.
...
- **IBM Cloud, Alibaba** have no surviving argument.
- **Wasabi** is viable but vendor-asserted rather than independently
  documented; behind Backblaze on evidence quality alone.
