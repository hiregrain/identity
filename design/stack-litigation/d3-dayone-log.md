...
1. **The product is a single sentence — "this record was really made, by
   ...
   actors — the operator itself, an insider with DB access, a poisoned
   CI/CD pipeline, a compromised cloud account — the chain-plus-checkpoint
...
2. **One failure is unfixable by any later investment: a log started at
   ...
   lands exactly when the company can least afford it — the first major
   dispute, the first partner audit, the first regulator inquiry — each of
   which asks about the *early* record, because early records are the
   oldest and most contested. "Our log covers everything since the first
...
6. **Concession and ruling aid:** if the founder can accept that the
   integrity of the year-0–2 record is permanently "trust us" — i.e., that
...
Genuinely cheap — dollars a month, an afternoon of code, a few hundred
...
- **No consistency proofs.** A Merkle tree answers "does this new head
  ...
  the interval, by someone with all the data — i.e., the operator. The
...
(standing, flags — projections, not facts); and the read log
...
would bloat the tree for no trust gain — it stays on the spine only).
...
- If the ecosystem stalls entirely, the fallback is N independent
  ...
  checkpoints anyone can archive — weaker than cosigning, still
...
3. **The log addresses none of the top in-band risk.** Party-attestation
   fraud (`06`) — fraudulent content, validly signed — sails straight into
...
6. **If Tessera the project stalls**, the fallback is a self-built tiled
   log on the identical storage layout — more work, same design. The
   commitment is to the artifact shape, not the dependency.

**Conditions under which the opposition's positions would be safe:**

- **Defer-to-v2 is safe iff the early record is disposable** — if the
  founder is prepared to tell the first cohort of workers, the first
