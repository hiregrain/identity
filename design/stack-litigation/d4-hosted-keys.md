...
1. **Third-party-verifiable non-repudiation is not load-bearing anywhere in
   ...
   the party signature mechanism as *open item 3* — unresolved, hence not
...
2. **The realistic threat ranking is lopsided.** Leaked key files /
   ...
   still live — GitGuardian). Malicious insiders are 5% of North American
...
5. **The hybrid (hosted-by-default + BYOK) is cheaper than either pure
   ...
   custody changes only who calls KMS — and because pure party-held
...
### 2a. What the signature is actually for — read the spec, not the vibes
...
exists because long-lived signer-held keys were the failure mode — "keys
get stored in CI secrets, shared across teams, and rarely rotated; a
leaked signing key compromises every artifact signed with it" — and its
replacement for key possession is **identity-bound ephemeral signing
recorded in a transparency log** ([Sigstore overview](https://docs.sigstore.dev/about/overview/)).
...
- **Onboarding**: each partner must generate an Ed25519 keypair correctly,
  ...
  partner — unstaffable — or (b) accept whatever they did, which is the
...
- **C4 — Party countersignature over its own issuance checkpoint.**
  ...
  countersigns — with a *party-held* low-frequency acknowledgment key or
  even a documented out-of-band approval — the Merkle root covering its
  issuance history. This restores a party-held cryptographic commitment at
...
(security-infra §3.5) already adopts for every other operator power — the
database itself, the registry, the read path. Hosted signing adds no
...
3. **The base-rate argument is an analogy, not a measurement.**
   ...
   strong, quantitatively loose — same caveat class as `06` §2.
