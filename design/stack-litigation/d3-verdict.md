...
records do not exist until the migration completes — between launch and
...
where the checkpoint chain requires the recomputation machinery — a
...
implementation choice): (1) **attestation commitments** — the
...
flags — projections, not facts); the read log (`PacketIssued` — worker
product surface, >80% of volume, zero public-trust gain; spine only).
...
- **D6 (ordering authority) — ruled with D3, as the docket directs.**
  ...
  each write — succeeded seamlessly by the tile log's checkpoint
...
- **D4 dependency, flagged for the D4 judge.** The hosted-keys brief
  ...
  condition is satisfied **in substance** from day one — every registry
  mutation, key event, and privileged operation is externally anchored
  in compliance-mode WORM within five minutes, and every KMS signature
  is in the cloud audit trail — but **witnessed co-signing arrives only
  at the deadline**. If the D4 court weighs hosted signing's insider
...
4. **Witness availability at the deadline.** No self-serve onboarding
   ...
   must be sourced bilaterally — auditor or the partner itself —
