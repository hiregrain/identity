...
writes across spine-DB and payload-DB. That means two-phase commit you wrote yourself,
...
region and ships the finished packet. That is one cross-region round trip, 150–250ms
...
outside the operator's control, witnessed externally**. This is D3's day-one
...
- *Self-verifying*: per-subject and per-party chain hashes let the migration
  prove byte-fidelity per stream at both ends. This is a verification luxury almost
...
1. *SQL-surface friction.* CRDB's Postgres compatibility is wire-level, not
   ...
   compatible subset. This means no triggers on migrating tables beyond the append-only
