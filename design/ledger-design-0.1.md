...
- **Opaque, because derived IDs break.** Any ID carrying embedded meaning
  ...
  underlying fact corrects. This is fatal in an append-only system where the
  ID is the join key for a career.
...
- **Vetting is tiered; the schema is not.** Internal verticals onboard under
  light review; external partners under proportionate diligence, meaning
  legal identity, business verification, an initial probationary scope (volume
...
| A vertical's customer | Nothing. No ledger read exists for customers; whatever a customer learns is vertical-local and the vertical's responsibility | None |
...
because "a model configuration has no career", and neither does an
